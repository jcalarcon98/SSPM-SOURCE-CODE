function PbDatePickerCtrl($scope, $log, widgetNameFactory, $element, $locale, $bsDatepicker) {
  ...
  if($scope.properties.minDate){
      if($scope.properties.minDate === 'today' ){
          this.minimunDate = $scope.properties.minDate;
      }else{
        this.minimunDate = new Date($scope.properties.minDate);
      }
  }
  
  if($scope.properties.maxDate){
     if($scope.properties.maxDate === 'today' ){
          this.minimunDate = $scope.properties.maxDate;
     }else{
        this.maximunDate = new Date($scope.properties.maxDate);
     }
  }
